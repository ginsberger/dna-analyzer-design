...
#include "../../CommandParams/SequenceAnalysisCommandsParams/FindAll/FindAllParams.h"
#include "../../CommandParams/SequenceAnalysisCommandsParams/Count/CountParams.h"
...
IParams* ParamsFactory::createParam(const std::string& commandName, const std::vector<std::string>& params)
{
    ...
    {
        return new FindAllParams(params);
    }
    if(commandName == "count")
    {
        return new CountParams(params);
    }
